...
public abstract class TileVPipeBase extends TileEntity implements ITileEntityBase, ITickable, IEmberPipeConnectable, IEmberPipePriority {
    ...
    protected boolean isAnySideUnclogged()
    {
        for (EnumFacing facing : EnumFacing.VALUES) {
            ...
            if (tile instanceof TileVPipeBase && !((TileVPipeBase) tile).clogged)
                return true;
        }
        return false;
    }
            ...
            if (packet > 0) {
                ...
                for (EnumFacing facing : EnumFacing.VALUES) {
                    ...
                    if (tile != null) {
                        ...
                        IEnergyStorage feHandler = tile.getCapability(CapabilityEnergy.ENERGY, facing.getOpposite());
                        ...
                        if(feHandler != null) {
                            possibleDirections.put(priority, facing);
                            emberCapabilities[facing.getIndex()] = feHandler;
                        } else if(tile instanceof TileVPipeBase) {
                            possibleDirections.put(priority, facing);
    ...
    private boolean pushStack(EnumFacing facing, Object handler) {
        ...
        if(handler instanceof IEnergyStorage) {
            IEnergyStorage feCapability = (IEnergyStorage) handler;
            double feAdded = feCapability.receiveEnergy((int)(packet * Options.mulitiplier), false);
            if(feAdded > 0) {
                packet = 0;
                return true;
            }
        ...
        if(handler instanceof TileVPipeBase) {
        	TileVPipeBase pipe = (TileVPipeBase) handler;
            if(pipe.packet <= 0) {
                pipe.packet = packet;
