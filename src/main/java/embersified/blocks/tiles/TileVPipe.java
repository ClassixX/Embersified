...
public class TileVPipe extends TileVPipeBase {
	...
	public EnumPipeConnection getConnection(IBlockAccess world, BlockPos pos, EnumFacing side) {
		...
		IEnergyStorage capFE = tile != null ? tile.getCapability(CapabilityEnergy.ENERGY, side.getOpposite()) : null;
		if (getInternalConnection(side) == EnumPipeConnection.FORCENONE) {
		...
		} else if (capFE != null && Options.pipesCanGenerateForge) {		//Enable pipes connecting to FE storage
			return EnumPipeConnection.BLOCK;
	...
	public void reverseConnection(EnumFacing face) {
		...
		if(tile instanceof TileVPipe)
			((TileVPipe) tile).updateNeighbors(world);
